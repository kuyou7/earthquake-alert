...
def monitor_earthquakes():
    ...
    # スペースを予約するために `st.empty()` を使用
    earthquake_info_placeholder = st.empty()
        ...
        if title:  # titleがNoneでない場合のみ処理を行う
            if title != last_earthquake_title:
                if "震度速報" in title or "震源情報" in title:
                    earthquake_info_placeholder.write(f"⚡地震発生検知！ {title}")
                    alert_user(title, link, description)
                    st.session_state.last_earthquake_title = title
                else:
                    earthquake_info_placeholder.write(f"取得しましたが対象外: {title}")
            else:
                earthquake_info_placeholder.write("新しい地震速報なし")
        else:
            earthquake_info_placeholder.write("地震情報を取得できませんでした")
